...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            print("❌ SignUpForm Errors:", form.errors)  # Debugging
        ...
        if 'profilePicture' in request.FILES or any(field in request.POST for field in form.fields):
            if form.is_valid():
                if not form.has_changed():
                    ...
                    # Only set profile.enableEmailNotifications if user’s email is verified
                    if user.email_verified:
...
def notifications(request):
    """
    Mark all notifications as read and display them in reverse order.
    """
...
def create_task(request):
    """
    Create a new Task (No Category or Email Notification).
    """
    ...
    if request.method == "POST":
        ...
        important = request.POST.get('important', 'Low')  # default is 'Low'

        # Convert dueDate to datetime
        try:
            due_datetime = datetime.strptime(dueDate, '%Y-%m-%dT%H:%M')
            due_datetime = timezone.make_aware(due_datetime, timezone.get_current_timezone())
        except ValueError:
            messages.error(request, 'Invalid date/time format.', extra_tags='createTaskError')
            return render(request, 'createTask.html', context)

        # Enforce minimum due date (5 minutes from now)
        min_due_datetime = timezone.now() + timedelta(minutes=5)
        if due_datetime <= min_due_datetime:
            Notifications.objects.create(
                name=(
                    'Due datetime must be at least 5 minutes from now. '
...
def running_tasks(request):
    """
    Show all running (PENDING or OVERDUE) tasks.
    """
...
def edit_task(request, task_id):
    """
    Edit an existing Task (No Category references).
    """
    try:
    ...
    if request.method == "POST":
        ...
        # Keep existing notificationTime/emailNotification if you want
        ...
        # Update Task fields (no category)
...
def completed_tasks(request):
    """
    Show tasks that have been completed.
    """
    search_query = request.GET.get('search', '')
    sort_by = request.GET.get('sort', 'completedDate')
    order = request.GET.get('order', 'desc')

    task_queryset = Task.objects.filter(user=request.user, status=Task.COMPLETED)
...
def clear_history(request):
    ...
    """
    completedTaskCount = Task.objects.filter(status=Task.COMPLETED, user=request.user).count()
    if completedTaskCount is None or completedTaskCount == 0:
        messages.error(request, 'Completed task list is empty', extra_tags='completedTaskError')
        return redirect('completed_tasks')

    Task.objects.filter(status=Task.COMPLETED, user=request.user).delete()
    Notifications.objects.create(
...
def delete_account(request, user_id):
    ...
    if user.is_superuser:
        ...
        messages.error(request, 'Admin account can only be deleted from the admin page.', extra_tags='profileInfoError')
        return redirect('profile')
...
def export_tasks(request):
    """
    Export tasks to CSV (No Category column).
    """
    ...
    writer.writerow(['Task Title', 'Description', 'Created Date', 'Due Date', 'Completed Date', 'Priority', 'Status'])
...
class ExportPDF(View):
    """
    Export tasks to PDF (No Category references).
    """
